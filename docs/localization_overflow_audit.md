...
| MEDIUM | `values-de` | `permission_denied` | 75 | 1.3x | Berechtigung verweigert. Bei Bedarf in den Einstellungen erneut aktivieren. |
| MEDIUM | `values-bn` | `permission_denied` | 74 | 1.3x | অনুমতি প্রত্যাখ্যান করা হয়েছে। প্রয়োজনে সেটিংস থেকে পুনরায় চেষ্টা করুন। |
...
| MEDIUM | `values-nl` | `permission_denied` | 73 | 1.3x | Toestemming geweigerd. Probeer het indien nodig via Instellingen opnieuw. |
| MEDIUM | `values-fa` | `geofence_hint` | 72 | 1.1x | یک منطقه امن یا منطقه خطر اضافه کنید تا حساسیت به‌صورت خودکار تنظیم شود. |
| MEDIUM | `values-hi` | `permission_denied` | 72 | 1.3x | अनुमति अस्वीकार कर दी गई। आवश्यकता होने पर सेटिंग्स से पुनः प्रयास करें। |
...
| MEDIUM | `values-es` | `permission_denied` | 65 | 1.1x | Permiso denegado. Intenta de nuevo desde Ajustes si es necesario. |
...
| MEDIUM | `values-it` | `permission_denied` | 64 | 1.1x | Autorizzazione negata. Riprova dalle Impostazioni se necessario. |
| MEDIUM | `values-pt` | `permission_denied` | 63 | 1.1x | Permissão negada. Tente novamente nas Definições se necessário. |
| MEDIUM | `values-pt` | `pin_setup_hint` | 63 | 1.3x | Crie um PIN para proteger a aplicação e os registos de eventos. |
| MEDIUM | `values-ru` | `permission_denied` | 63 | 1.1x | Разрешение отклонено. При необходимости повторите в настройках. |
| MEDIUM | `values-ur` | `permission_denied` | 63 | 1.1x | اجازت رد کر دی گئی۔ اگر ضروری ہو تو سیٹنگز سے دوبارہ کوشش کریں۔ |
...
| MEDIUM | `values-fr` | `permission_denied` | 60 | 1.1x | Autorisation refusée. Réessayez dans les Réglages si besoin. |
